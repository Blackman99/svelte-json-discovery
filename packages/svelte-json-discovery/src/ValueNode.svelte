...
<script lang='ts'>
    ...
    const RendererComponent = $derived(customRenderer?.renderer.component);
    const RendererSnippet = $derived(customRenderer?.renderer.snippet);
    const rendererProps = $derived<JsonViewerRendererProps>({
        node,
        density: 'full',
        controller: api.controller,
    });
    const hasPluginActions = $derived(api.hasPluginActions(node));
    ...
    function reportRendererError(error: unknown) {
        if (customRenderer) {
            api.reportPluginError(customRenderer.pluginId, node, 'renderer', error);
        }
    }
</script>
